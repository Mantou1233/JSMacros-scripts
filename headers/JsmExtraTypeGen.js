...
const fromRegistry = {
  Biome: "field_41156", //"field_25933", // is changing in 1.19.4 probably
  Sound: "field_11156",
  ItemId: "field_11142",
  BlockId: "field_11146",
...
const Registries = Java.type("net.minecraft.class_2378");
const BuiltinRegistries = Java.type("net.minecraft.class_7923");
...
for (const type in fromRegistry) {
  Chat.log(fromRegistry[type]);
  temp = Java.from(
    (fromRegistry[type] in Registries ? Registries : BuiltinRegistries)[
      fromRegistry[type]
    ]
      .method_10235()
      .toArray()
...
temp = temp
  ...
  .map((code) =>
    InputUtil$Type[code <= 7 ? "field_1672" : "field_1668"].method_1447(code)
  )
...
temp = temp // order
  ...
  .concat(
    temp.filter(
      (k) => !funcRegex.test(k) && !commonRegex.test(k) && !groupRegex.test(k)
    )
  );
replaceToFile("Key");
log(`fetched ${temp.length} keys for Key`);

temp = Java.from(Client.getGameOptions().getRaw().field_1839).map((k) =>
  k.method_1431()
);
replaceToFile("Bind");
...
temp = Java.from(Client.getRegistryManager().getItemIds())
  .map((id) => empty(id).getTags())
  .flat()
  .sort()
  .filter((v, i, a) => v !== a[i - 1]);
...
// const BlockHelper = Java.type(
//   "xyz.wagyourtail.jsmacros.client.api.helpers.BlockHelper"
// );
// const BlockRegistry = Registries.field_11146;
