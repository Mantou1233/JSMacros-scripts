const options = {
	...
	// other
	meloTypes: false // include the types used in my scripts
};
...
async function main() {
	...
	const jsmFile = dir
		.filter(f => jsmFileReg.test(f))
		.sort()
		.at(-1);
	if (!dir.includes("Graal.d.ts")) throw "can't find Graal.d.ts file!";
	if (!jsmFile) throw "can't find JsMacros.d.ts file!";
	if (options.extraTypes && !dir.includes("JsmExtra.d.ts"))
		throw "can't find JsmExtra.d.ts file!";
	if (options.meloTypes && !dir.includes("MeloTypes.d.ts"))
		throw "can't find MeloTypes.d.ts file!";
	...
	if (options.dictifyJavaTypes) {
		...
		const singleRegex =
			/export function type\(className: ("[\w\.$]+")\): (.+)/gm;
		for (let i = 1; i >= 0; i--) {
			console.log(`dictifyJavaTypes: ${i ? "graal" : "jsm"}`);
			const to = `    export function type<C extends keyof JavaTypeDict>(className: C): JavaTypeDict[C]
}
...
${(i ? graal : jsm).match(regex)?.[0].replace(singleRegex, "$1: $2")}`;
	...
	if (options.extraTypes) {
		...
		jsm = jsm.replace(
			/ {16}namespace api \{(?:.*\n)*? {16}\}/,
			api.replace(/^/gm, " ".repeat(16))
		);
	}
	if (options.fixJsmTypes || options.shortifyTypes) {
		console.log(
			"fixJsmTypes: jsm" +
				(options.fixJsmTypes ? "" : ", overrided by shortifyTypes")
		);
		jsm.match(/(?<=\.)[\w$]+\$\w/g)
			.sort()
	...
	if (options.fixEventTypes) {
		...
		jsm =
			...
			match
				...
				.replace(
					/^    interface ([A-Za-z]+) extends BaseEvent \{$/gm,
					"    $1: {"
				)
				.replace(/\n    export(?:[^}]+\n)    \}/g, "") +
			"\n\n\n" +
...
` + graal.trimStart();
		...
		const missingExtends = {
			...
			NBTElementHelper:
				"NBTElementHelper$NBTCompoundHelper & NBTElementHelper$NBTListHelper & NBTElementHelper$NBTNumberHelper"
		};
		const temp = jsm
			.match(/^                    .+$/gm)
			.map(s => s.slice(19))
			.filter(s =>
				s.includes(
					"_javatypes.xyz.wagyourtail.jsmacros.client.api.helpers"
				)
			)
			.join();
		const redirectNeeded = temp
			.match(
				/_javatypes\.xyz\.wagyourtail\.jsmacros\.client\.api\.helpers(?:\.[\w$]+)+/g
			)
		...
		const redirectArgsNeeded = temp
			.match(
				/_javatypes\.xyz\.wagyourtail\.jsmacros\.client\.api\.helpers(?:\.[\w$]+)+(?= *<(?!(?:\/\*.*?\*\/|any|,| )+>))/g
			)
			.map(m => m.match(nameRegex)?.[0])
			.sort()
			.filter((v, i, a) => v && v !== a[i - 1])
			.map(s => (s in overrides ? overrides[s] : s));
		const T = ["", "<T>", "<T, U>", "<T, U, R>", "<T, U, R, C>"];
			...
			if (redirectNeeded.includes(name)) {
				...
				jsm = jsm.replace(
					RegExp(
						"^(                    .*)" +
							forRegExp +
							(redirectArgsNeeded.includes(name)
								? ""
								: "(?: *<(?:\\/\\*.*?\\*\\/|any|,| )+>)?"),
						"gm"
					),
		...
		const maxLength = Object.values(types).reduce(
			(p, v) => Math.max(p, (v.name + Tany[v.Targs]).length),
			0
		);
		const maxRedirLength = Object.keys(redirects).reduce(
			(p, v) =>
				Math.max(
					p,
					(
						v +
						(redirectArgsNeeded.includes(v)
							? Tany[redirects[v]]
							: "")
					).length
				),
...
${Object.keys(types)
	...
	.map(
		t =>
			`type ${(types[t].name + Tany[types[t].Targs]).padEnd(
				maxLength
			)} = ${
				types[t].Targs || types[t].name in missingExtends ? "  " : "_&"
			}${t + T[types[t].Targs]}${
				types[t].name in missingExtends
					? " & " + missingExtends[types[t].name]
					: ""
			}`
	)
	.join("\n")}${
					  ...
					  Object.keys(redirects)
							...
							.map(
								n =>
									`    type $${(
										n +
										(redirectArgsNeeded.includes(n)
											? Tany[redirects[n]]
											: "")
									).padEnd(maxRedirLength)} = ${n}${
										redirectArgsNeeded.includes(n)
											? T[redirects[n]]
											: ""
									}`
...
` + jsm.trimStart();
	...
	await rmd;
	if (options.extraTypes)
		await fs.mkdir("./output/node_modules/@types/jsmextra", {
			recursive: true
		});
	if (options.meloTypes)
		await fs.mkdir("./output/node_modules/@types/melotypes", {
			recursive: true
		});
	await fs.mkdir("./output/node_modules/@types/graal", { recursive: true });
	await fs.mkdir("./output/node_modules/@types/jsmacros", {
		recursive: true
	});
	await Promise.all([
		...
		fs.writeFile(
			`./output/node_modules/@types/graal/package.json`,
			JSON.stringify(
				{
					name: "@types/graal",
					version: "1.0.0",
		...
		fs.writeFile(
			`./output/node_modules/@types/jsmacros/package.json`,
			JSON.stringify(
				{
					name: "@types/jsmacros",
					version,
		...
		...(!options.extraTypes
			...
			: [
					fs.copyFile(
						"./JsmExtra.d.ts",
						"./output/node_modules/@types/jsmextra/JsmExtra.d.ts"
					),
					fs.writeFile(
						`./output/node_modules/@types/jsmextra/package.json`,
						JSON.stringify(
							{
								name: "@types/jsmextra",
								version: "1.0.0",
								description:
									"Extra TypeScript definitions for JsMacros",
								homepage:
									"https://github.com/aMelonRind/JsMacros-shared-script/tree/main/JsmTypes",
								license: "MIT",
								types: "JsmExtra.d.ts"
							},
							undefined,
		...
		...(!options.meloTypes
			...
			: [
					fs.copyFile(
						"./MeloTypes.d.ts",
						"./output/node_modules/@types/melotypes/MeloTypes.d.ts"
					),
					fs.writeFile(
						`./output/node_modules/@types/melotypes/package.json`,
						JSON.stringify(
							{
								name: "@types/melotypes",
								version: "1.0.0",
								description:
									"TypeScript definitions for my scripts",
								homepage:
									"https://github.com/aMelonRind/JsMacros-shared-script/tree/main/JsmTypes",
								license: "MIT",
...
function getExtraTypesReplaces() {
	return {
		event: `DimensionChange.dimension: string;
dimension: Dimension;
...
button: ClickSlotButton;`.split("\n"),
		library: `Player.getGameMode(): string;
...
getBiomeAt(x: number, z: number): Biome;`.split("\n"),
		helper: `BossBarHelper.getColor(): string;
...
getId(): StatusEffectId;`.split("\n"),
		class: `Inventory.click(slot: number, mousebutton: number): _javatypes.xyz.wagyourtail.jsmacros.client.api.classes.Inventory<T>;
