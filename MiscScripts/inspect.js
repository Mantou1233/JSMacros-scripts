...
const LivingEntityHelper = Java.type(
          ...
          JavaWrapper.methodToJava((txt) => {
            ...
            try {
              while (nbt && (path[1] || /^\d+$/.test(path[0]))) {
                if (/^\d+$/.test(path[0]))
                  nbt = nbt.get(parseInt(path.shift()));
            ...
            if (nbt?.isCompound() && path[0]) {
              ...
              if (keys.contains(path[0])) nbt = nbt.get(path[0]);
                ...
                const filtered = keys
                  .toArray()
                  .filter((k) => k.startsWith(path[0]));
                if (filtered.length === 1) {
                  nbt = nbt.get(filtered[0]);
                  if (lastNbtPath.endsWith(" "))
                    // press space to autocomplete
                    nbtInput.setText(
                      lastNbtPath.trimEnd() +
                        filtered[0].slice(path[0].length) +
                        "."
                    );
...
function addItem(x, y, item, textIfEmpty = "No Item!") {
  if (item instanceof NBTElementHelper)
    item = ItemStack.method_7915(item.getRaw()); // .fromNbt()
  if (item && !(item instanceof ItemStackHelper))
    item = new ItemStackHelper(item);
...
function getItem(item) {
  return () => {
    if (Player.getPlayer().getRaw().method_5687(2)) {
      // .hasPermissionLevel
      const cmd = `/${bukkit ? "minecraft:" : ""}give @s ${item.getItemId()}${
          nbtHelperToString(item.getNBT()) ?? ""
        }`;
        if(cmd > 256) 
            return Chat.log("Command is too long!")
      Chat.say(
        cmd
      );
    }
 ...
 */
function nbtHelperToString(nbt){
    return nbt?.toString()?.match(/^[^\{]*(\{.*\})$/)?.[1].slice(1).slice(0, -1) || "{}";
}